...
class TestOeApiFactory(object):
    def test_create_oe_api_raises(self):
        oe = MagicMock()
        oe.hardware_id = CpoHardwareId(oe_id=OeId.NVIDIA_SP6, elsfp_id=None)
...
class TestElsfpApiFactory(object):
    def test_create_elsfp_api_raises_when_id_none(self):
        elsfp = MagicMock()
        elsfp.hardware_id = CpoHardwareId(oe_id=OeId.NVIDIA_SP6, elsfp_id=None)
    ...
    def test_create_elsfp_api_raises_when_id_set(self):
        elsfp = MagicMock()
        elsfp.hardware_id = CpoHardwareId(oe_id=OeId.NVIDIA_SP6, elsfp_id=ElsfpId.EXAMPLE)
        factory = ElsfpApiFactory(elsfp)
        with pytest.raises(ValueError):
            factory.create_elsfp_api()

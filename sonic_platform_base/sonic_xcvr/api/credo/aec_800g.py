"""
    ...
    Includes CMIS target firmware upgrade support for upgrading remote targets
    from the local target itself.
"""
...
class CredoAec800gApi(CmisApi):
    def set_firmware_download_target_end(self, target):
        """
        Sets the firmware download target end to the specified target.

        Args:
            target: Supported target identifier. Must be one of
                TARGET_E0_VALUE, TARGET_E1_VALUE, or TARGET_E2_VALUE.

        If the target mode is set to a remote target, then the page select byte is set to 0.
        Also, the remote target is then checked to ensure that it is accessible.
        In case of any error, the target mode is restored to E0.

        Returns:
            bool: True if the target mode is set successfully, False otherwise.
        """
        if target not in TARGET_LIST:
            logger.error("Unsupported firmware download target: {}".format(target))
            return False
        try:
            ...
            if target != TARGET_E0_VALUE:
                if not self.xcvr_eeprom.write(consts.PAGE_SELECT_BYTE, 0):
                    logger.error("Failed to set page select to 0 for target mode {}".format(target))
                    return self._restore_target_to_E0()

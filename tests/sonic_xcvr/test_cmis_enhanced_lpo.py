...
class TestCmisEnhancedLpoApi:
    ...
    @pytest.mark.parametrize("lpo_reads, expected_lpo", [
        (
            # All fields readable
            {
                ...
                # bits 3-0 = 0x03 -> 3 * 5 = 15 mV
                lpo.LPO_TX_HOST_INPUT_VMA_MON_ACCURACY: 0b11110011,
                # bits 3-0 = 0x05 -> 5 * 0.2 = 1.0 dB
                lpo.LPO_RX_INPUT_OMA_MON_ACCURACY: 0b11110101,
                lpo.LPO_TX_OUTER_EXTINCTION_RATIO_MAX: 50,  # 50 * 0.1 = 5.0 dB
            },
            {
                lpo.LPO_TX_POLARITY_INVERTED: 0b10000001,
                lpo.LPO_RX_POLARITY_INVERTED: 0b01000010,
    ...
    def test_get_transceiver_dom_real_value(self, mock_super,
        ...
        read_map = {f: v for f, v in zip(vma_fields + oma_fields, vma_raws + oma_raws)}
    ...
    def test_get_transceiver_dom_flags_unpack(self, mock_super, bitmask, expected_lanes):
        self.api.xcvr_eeprom.read = MagicMock(return_value=bitmask)
    ...
    @patch('sonic_platform_base.sonic_xcvr.xcvr_api_factory.XcvrApiFactory._get_vendor_name',
           MagicMock(return_value='Arista'))
    @patch('sonic_platform_base.sonic_xcvr.xcvr_api_factory.XcvrApiFactory._get_vendor_part_num',
